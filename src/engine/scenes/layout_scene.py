...
class LayoutScene(Scene):
    ...
    def do_create(self):
        ...
        self.current_score_surface = create_text(self.ecs_world, str(self.score[0]), 8, pygame.Color(255, 255, 255), pygame.Vector2(52, 10), TextAlignment.LEFT)
        self.high_score_surface = create_text(self.ecs_world, str(self.high_score[0]), 8, pygame.Color(255, 255, 255), pygame.Vector2(112, 10), TextAlignment.CENTER)
        super().do_create()
    ...
    def do_draw(self, screen):
        ...
        system_rendering_texts(self.ecs_world, self.screen)
    ...
    def draw_lives(self, screen):
        ...
        for i in range(self.max_lives[0] - 1):
            x = 0 + (i * 15)
            screen.blit(frame_image, (x, 20))

    def lose_lives(self):
        self.max_lives[0] = max(0, self.max_lives[0] - 1)
    ...
    def draw_plane_counter(self, screen):
        ...
        current_frame = self.plane_counter_stage[0]
    ...
    def damage_plane_counter(self, amount=1):
        self.plane_counter_stage[0] = max(0, self.plane_counter_stage[0] - amount)

    def reset_max_lives(self):
        self.max_lives[0] = 4

    def reset_plane_counter(self):
        self.plane_counter_stage[0] = 40
